<?php  
	...
	if($select) {
		...
		if($insert){
			$return = $conn->query("SELECT json_object('insert',true,'car',json_object('id',id,'user',(select json_object('id',$id_user)),'carBrand',(select json_object('name',nombre) from marca_coche where id = c.marca_coche), 'carModel', (select json_object('name',nombre,'engine',motor) from modelo_coche where id = c.modelo_coche))) as data from coche c where marca_coche = $id_marca and modelo_coche = $id_modelo and usuario = $id_user and id=(select max(id) from coche where marca_coche = $id_marca and modelo_coche = $id_modelo and usuario = $id_user)");
			if($return)
